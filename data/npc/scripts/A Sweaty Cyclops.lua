...
function soulorbs(cid, message, keywords, parameters, node)
    if(not npcHandler:isFocused(cid)) then
        return false
    end
        if getPlayerItemCount(cid,5944) >= 10 then
        if doPlayerRemoveItem(cid,5944,10) then
            npcHandler:say('Here is your item!', cid)
            doPlayerAddItem(cid,6529,30)
        end
        else
            npcHandler:say('You need 10 soul orbs to trade them with me!', cid)
   end   
end
...
keywordHandler:addKeyword({'help'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I can forge Amulet, Obsidian Knife, Huge Chunk of Crude Iron, Piece of Draconian Steel or melt a gold ingot!"})

local node1 = keywordHandler:addKeyword({'amulet'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Hum Humm! Welcume lil. Me can do unbroken but Big Ben want gold 5000...Yes or no??'})
...
local node5 = keywordHandler:addKeyword({'infernal bolt'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want me to change 10 soul orbs for 30 infernal bolts?'})
    node5:addChildKeyword({'yes'}, soulorbs, {npcHandler = npcHandler, onlyFocus = true, reset = true})
    node5:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got the 10 soul orbs.', reset = true})

local node6 = keywordHandler:addKeyword({'melt'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want me to melt your gold ingot?'})
    node6:addChildKeyword({'yes'}, melt, {npcHandler = npcHandler, onlyFocus = true, reset = true})
